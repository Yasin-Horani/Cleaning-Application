...
public class EditOneTaskActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_edit_one_task);
        Context context = this;

        // get task id.
        Intent intent = getIntent();
        int taskId = intent.getIntExtra("TASK_ID", -1);
        task = Task.getById(context, taskId);
        if (task == null) {
        ...
        // naam
        taskName = findViewById(R.id.editOneTask_taskName_pt_id);
        taskName.setText(task.getNaam());
        ...
        roomsSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                Toast.makeText(EditOneTaskActivity.this, "Geselecteerde Kamer: " + room.getName(), Toast.LENGTH_SHORT).show();
            }
        ...
        // Set room spinner to task's room
        for (int i = 0; i < roomList.size(); i++) {
            if (roomList.get(i).getId() == task.getRoomId()) {
        ...
        usersSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                Toast.makeText(EditOneTaskActivity.this, "Geselecteerde Gebruiker: " + user.getGebruikersnaam(), Toast.LENGTH_SHORT).show();
            }
        ...
        // Set user spinner to task's user
        for (int i = 0; i < userList.size(); i++) {
            if (userList.get(i).getId() == task.getUserId()) {
                usersSpinner.setSelection(i);
                break;
            }
        }
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, intervalOptions);
        ...
        // Set interval spinner to task's interval
/*        for (int i = 0; i < intervalOptions.size(); i++) {
            if (intervalOptions.get(i).equals(task.getInterval())) {
        ...
        taskDescriptionEditText.setText(task.getBeschrijving());
        ...
        findViewById(R.id.editOneTask_updateOneTask_btn_id).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                task.setNaam(name);
                task.setDatum(startDatum);
                task.setRoomId(room.getId());
                task.setUserId(user.getId());
               // task.setInterval(intervalSelectedItem);
                task.setBeschrijving(taskDescription);
                task.update(context,task);
                Toast.makeText(EditOneTaskActivity.this, "Taak is succesvol gewijzigd.", Toast.LENGTH_SHORT).show();
                finish();
                Intent intent = new Intent(EditOneTaskActivity.this, EditingTasksActivity.class);
